...
#Let's look at the gender balance between the population and those who work for political parties#
...
d <- ESS %>% group_by(cntry, gender) %>% filter(!is.na(wparty)) %>%
  count(wparty) %>% mutate(perc_wk_pp=n/sum(n)*100) %>%
  filter(!wparty=="have not worked for pp") %>%
  select(-wparty)
...
left_join(c, d, by=c("cntry", "gender")) %>% select(-n.x, -n.y) %>%
  knitr::kable("pandoc", caption = "Gender Balance in Population and Workers of Parties in European Democracies",
               col.names = c('Country', 'Gender', 'Percentage Population', 'Percentage Worker of Political Party'),
